...
class MsgSerializer:
    ...
    def to_bytes(data: dict) -> bytes:
        """
        只打包一个对象：顶层一定是 dict（请求或响应）。
        numpy.ndarray 和 ModalityConfig 使用自定义编码。
        """
    ...
    def from_bytes(data: bytes) -> dict:
        """
        只解析第一个 msgpack 对象：
        - 使用 Unpacker，可以容忍 data 里有多余的字节（例如前缀/多个对象）
        - 只取第一个对象作为请求，必须是 dict
        """
        unpacker = msgpack.Unpacker(
            raw=False,
            object_hook=MsgSerializer.decode_custom_classes,
            strict_map_key=False,  # 允许非 str 的 map key（防止奇怪数据炸掉）
        )
        unpacker.feed(data)
        ...
        try:
            ...
            raise RuntimeError("MsgSerializer.from_bytes: no objects unpacked from data")

        if not isinstance(obj, dict):
            raise TypeError(
                f"MsgSerializer.from_bytes: first unpacked object is not dict, got {type(obj)}"
            )
...
class BaseInferenceServer:
    ...
    def run(self):
        ...
        while self.running:
            try:
                # ZMQ REQ/REP：一次只收一个 bytes
                message = self.socket.recv()
                print(f"[Server] Received raw msg len: {len(message)}")

                request = MsgSerializer.from_bytes(message)

                if not isinstance(request, dict):
                    raise TypeError(f"Decoded request is not a dict: {type(request)}")

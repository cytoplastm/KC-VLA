...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        keyframes: dict[int, list[int]] | None = None,   # 🔴 新增参数，默认 None
    ):
        """
    ...
    def compute_obs_frames(self, trajectory_id: int, base_index: int) -> tuple[list[int], list[int]]:
        ...
        used = [k for k in kfs if k <= base_index]

        if not used or used[-1] != base_index:
            used.append(base_index)

        if len(used) > MAX_OBS_FRAMES:
            frames = used[-MAX_OBS_FRAMES:]
        else:
            frames = used
        ...
        if len(frames) < MAX_OBS_FRAMES:
            pad_len = MAX_OBS_FRAMES - len(frames)
            frames = [0] * pad_len + frames
            mask   = [0] * pad_len + mask

        assert len(frames) == MAX_OBS_FRAMES
        assert len(mask) == MAX_OBS_FRAMES
        return frames, mask
    ...
    def get_video_multi(
        ...
        """
        根据一组绝对帧号 frame_indices，取出对应的视频帧。

        返回:
            np.ndarray，shape = (T_obs, H, W, C)
            其中 T_obs = len(frame_indices)
        """
        # 确保是 video.*
        assert key.startswith("video."), f"Video key must start with 'video.', got {key}"

        # 1) 处理帧号，裁剪到 [0, traj_len-1] 范围内，防止越界
        trajectory_index = self.get_trajectory_index(trajectory_id)
        traj_len = self.trajectory_lengths[trajectory_index]

        step_indices = np.asarray(frame_indices, dtype=int)  # [T_obs]
        step_indices = np.maximum(step_indices, 0)
        ...
        # 3) 用 timestamp 把「逻辑帧 index」映射到视频时间戳
        assert self.curr_traj_data is not None, f"No data found for trajectory_id={trajectory_id}"
        assert "timestamp" in self.curr_traj_data.columns, f"No timestamp found in trajectory_id={trajectory_id}"

        timestamp: np.ndarray = self.curr_traj_data["timestamp"].to_numpy()  # shape [T_total]
        video_timestamp = timestamp[step_indices]  # shape [T_obs]

        # 4) 调用通用的 get_frames_by_timestamps，从视频里抽这些时间点最近的帧
        ...
        # frames: np.ndarray, 形状一般是 [T_obs, H, W, C]

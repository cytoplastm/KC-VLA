...
#关键帧版本
def get_attention_mask_for_frame(self, frame_data):
    """
    根据是否有效的逻辑生成每一帧的掩码，1表示有效，0表示无效。
    """
    mask = np.ones_like(frame_data)  # 默认所有帧都是有效的
    if not self.is_valid_frame(frame_data):  # 判断该帧是否有效
        mask = np.zeros_like(frame_data)  # 无效帧标记为 0
    return torch.tensor(mask)

def get_frame_with_mask(frame_data):
    """
    将视频帧数据和对应的掩码结合，返回加了掩码的帧数据。
    """
    # 获取当前帧对应的掩码
    mask = get_attention_mask_for_frame(frame_data)

    if mask.sum() == 0:
        frame_data = torch.zeros_like(frame_data)  # 如果掩码为 0，就将帧置为全零
...
def is_valid_frame(self, frame_data):
    """
    判断视频帧是否有效的逻辑。你可以根据 `observation_indices` 或其他标准来判断。
    """
    # 示例：基于 `observation_indices` 进行判定
...
class GR00TTransform(InvertibleModalityTransform):
    ...
    def _apply_vlm_processing(self, batch: dict) -> BatchFeature:
        ...
        full_prompt = (
            f"Task: {lang}\n"
            "Note: The provided images are a chronological sequence of **image pairs**. "
            "For each time step, two images are provided: the first is the **Third-Person View**, and the second is the corresponding **Wrist View**.\n"
            "The final pair represents the current state, while all preceding pairs are history.\n"
            "Based on this multi-view sequence, analyze the robot's progress and determine the immediate next action."
        )
        ...
        text_content.append({"type": "text", "text": full_prompt})
    ...
    def _prepare_video(self, data: dict):
        ...
        if obs_mask is not None:
            # 确保是 numpy bool 向量
            obs_mask = np.asarray(obs_mask, dtype=bool)      # [T_obs]
            valid_frames = np.nonzero(obs_mask)[0]           # 等价于 torch.nonzero(..., as_tuple=True)[0]

            # 只保留有效帧（T 这个维度）
            images = images[:, valid_frames, ...]            # [V, T_valid, C, H, W]
    ...
    #     输入:
    #         data["video"]: 形状 [T_obs, V, H, W, C]，这里 T_obs 通常就是 4（已经在 Dataset 里补好了）
    #         data["obs_mask"]: 可选, [T_obs]，1=有效帧, 0=补出来的无效帧

    #     输出:
    #         images: np.uint8, [V, T_obs, C, H, W]，不再根据 obs_mask 删帧
    #         video_mask: np.bool_, [T_obs]，给后面 action head 用（现在可以简单等于 obs_mask 或全 1）
    #     """
    #     obs_mask = data.get("obs_mask", None)  # [T_obs] or None
    ...
    #         # 训练时候也可以简单设为全 1
    #         T = images.shape[1]
    #         video_mask = np.ones((T,), dtype=bool)
    #     else:
    #         video_mask = np.asarray(obs_mask, dtype=bool)

    #     return images, video_mask
    ...
    def apply_single(self, data: dict) -> dict:
        ...
        # ⭐ 新增：拿出 obs_mask（长度 = T_obs）
        obs_mask = data.get("obs_mask", None)  # np.array([T_obs]) or None
        if obs_mask is not None:
           obs_mask = np.asarray(obs_mask, dtype=bool)   # [T_obs]

        # =========================================================
        # 🟢 [新增] 透传 keyframe_counts
        # =========================================================
        # 从 dataset.get_step_data 返回的 data 中提取
        ...
        # ⭐ 把 obs_mask 也塞进 transformed_data（留给 action_head 用）
        if obs_mask is not None:
            transformed_data["obs_mask"] = obs_mask  # [T_obs]

...
class FlowmatchingActionHead(nn.Module):
    ...
    def forward(self, backbone_output: BatchFeature, action_input: BatchFeature) -> BatchFeature:  
        ...
        model_output = self.model(
            ...
            encoder_attention_mask=vl_bool_mask,     # DiT Cross-Attention
    ...
    def get_action(self, backbone_output: BatchFeature, action_input: BatchFeature) -> BatchFeature:
        ...
        # Align inference-time state padding handling with training.
        if state_mask.ndim == 3:
            state_time_mask = state_mask.abs().sum(dim=-1) > 0
        else:
            state_time_mask = state_mask > 0

        batch_size = vl_embs.shape[0]
        device = vl_embs.device
        T_future = self.config.num_target_vision_tokens
        future_time_mask = torch.ones((batch_size, T_future), device=device, dtype=torch.bool)
        action_time_mask = torch.ones(
            (batch_size, self.config.action_horizon), device=device, dtype=torch.bool
        )
        sa_bool_mask = torch.cat(
            [state_time_mask.to(torch.bool), future_time_mask, action_time_mask],
            dim=1,
        )

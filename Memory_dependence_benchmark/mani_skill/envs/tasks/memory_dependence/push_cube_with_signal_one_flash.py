...
class PushCubeWithSignalEnv(BaseEnv):
    ...
    def _build_led(self):
        ...
        builder.add_cylinder_visual(
            ...
            pose=sapien.Pose(p=[0, 0, z_base_1], q=rotate_to_z)
        )
        ...
        builder.add_cylinder_visual(
            ...
            material=self.mat_base, 
            pose=sapien.Pose(p=[0, 0, z_base_2], q=rotate_to_z)
        )
        builder.add_cylinder_collision(
            radius=r_base_2, half_length=h_base_2, 
            pose=sapien.Pose(p=[0, 0, z_base_2], q=rotate_to_z)
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            # Phase 0: Initial OFF (4-6s)
            t0 = (torch.rand(b, device=self.device) * 2.0 + 4.0) * freq
            
            # Phase 1: ON (4-6s)
            t1 = (torch.rand(b, device=self.device) * 2.0 + 4.0) * freq
            
            # 我们只需要定义前两个阶段，结束后进入 Phase 2 (Task Ready)
            self.phase_durations = torch.stack([t0, t1], dim=1).long()
    ...
    def _before_control_step(self):
        # 只要 Phase < 2，说明还在倒计时阶段
        not_done = self.current_phase < 2
        ...
        if len(active_indices) > 0:
            ...
            # 当 Phase 达到 2 时，任务开始
            self.task_ready = (self.current_phase >= 2)
            self._update_led_visuals()

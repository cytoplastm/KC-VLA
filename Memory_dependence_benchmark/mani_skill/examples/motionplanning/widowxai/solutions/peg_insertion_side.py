...
def solve(env: PegInsertionSideEnv, seed=None, debug=False, vis=False):
    ...
    offset = sapien.Pose([0, 0, -max(0.05, env.peg_half_sizes[0, 0].item() / 2 + 0.01)])
    ...
    offset = sapien.Pose([-0.01, 0, -0.01 - env.peg_half_sizes[0, 0].item()])
    ...
    res = planner.move_to_pose_with_screw(insert_pose * (sapien.Pose([0, 0, 0.03])))
    if res == -1: return res
    planner.close()
    return res
